...
public class CoffeeShopBeforeMain {
	
	public static int ORDER  = 10;
	private static final Random random = new Random();
	
	public static void testBefore(int order) {
		
		ORDER = order;
		CoffeeShop shop = new CoffeeShop();
		
		for(int i=0;i<ORDER;i++) {
			int tableNumber = random.nextInt(ORDER);
			shop.takeOrder(CoffeeFlavour.generateRandomCoffeeFlavour(),CoffeeLatteArt.generateRandomCoffeeLatteArt(),tableNumber);
		}
